...
export default async function (wptServer) {
  try {
    const wptDataResponse = await fetch(
      `http://${wptServer}//getTesters.php?f=json`
    );
    ...
    if (wptData.statusCode === 200) {
      ...
      errorLogger(
        `Error getWPTClusterDetails.js getWPTClusterDetails Error: did not receive 200 response from WPT`
      );
      return {};
    }
  } catch (error) {
